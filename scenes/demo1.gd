...
@export var to_use_mouse_to_draw = false
@export var to_draw_triangles = true
@export var to_draw_triangle_edges = false
@export var to_draw_voronoi_cells = false
@export var to_draw_voronoi_cells_convex_hull = false
@export var to_draw_voronoi_edges = false
@export var to_draw_points = false
@export var to_draw_triangle_centers = false
@export var debug_mode = false

@onready var input_points = $MarginContainer/HBoxContainer/PointsTextEdit
...
func _input(event):
	if to_use_mouse_to_draw and event is InputEventMouseMotion:
		moving = true
...
func _draw():
	if to_draw_triangles: draw_triangles(points, delaunay)

	if to_draw_triangle_edges: draw_triangle_edges(points, delaunay)

	if to_draw_voronoi_cells: draw_voronoi_cells(points, delaunay)

	if to_draw_voronoi_cells_convex_hull: draw_voronoi_cells_convex_hull(points, delaunay)

	if to_draw_voronoi_edges: draw_voronoi_edges(points, delaunay)

	if to_draw_points: draw_points()

	if to_draw_triangle_centers: draw_triangle_centers()
...
func get_random_points(seed_points = default_seed_points):
	...
	for i in range(new_points.size()):
		if i >= initial_points.size():
			var new_point = Vector2(randi() % int(viewport_size.x), randi() % int(viewport_size.y))
